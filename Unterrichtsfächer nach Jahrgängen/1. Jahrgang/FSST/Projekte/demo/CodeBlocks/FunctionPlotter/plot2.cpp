...
int main()
{
   ...
   int f2col =COLOR(50,200,50);
   int f3col =COLOR(200,200,50);

   setbkcolor(bkcol);                  // Farben setzen
   cleardevice();
   setcolor(acol);
   outtextxy(50,15," Diagramm  y=f(x) ");
   ...
   setcolor(f3col);
   moveto(x1,y0);
   for (xf=xs1;xf<=xs2;xf+=0.025)       // Diagramm - Sinusüberlagerung
   {
       yf = function3(xf);
       x=scalx(x1,x2,xs1,xs2,xf);
       y=scaly(y1,y2,ys1,ys2,yf);
...
{   float y=0;
    ...
    for ( f=1;f<=99;f+=2 )
    {
       y += 1.0/f*sin(f*x);                // Sinusüberlagerung
    }
    return y;
}
