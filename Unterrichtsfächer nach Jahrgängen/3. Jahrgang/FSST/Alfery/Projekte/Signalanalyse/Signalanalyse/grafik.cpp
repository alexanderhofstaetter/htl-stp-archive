...
void grafik::paintEvent( QPaintEvent * )
{
    ...
    int h = height();                            //          Fensterh?he   abfragen
    ...
    g.setWorldMatrix( matrix );                  // Koordinaten-Transformation durchf?hren

    g.setPen(QColor(100,100,250));
    int x,y,x0,y0;
    x0=0; y0=0;
    for(x=0;x<N;x++)                            // Sample-Array #1 als Linienzug darstellen
    {   y = - (int) samples[x];
    ...
    x0=0; y0=0;
    for(x=0;x<N;x++)                            // Sample-Array #2 als Linienzug darstellen
    {   y = - (int) samples2[x];
        g.drawLine(x0,y0,x,y);
        x0=x;y0=y;
    }
    ...
    g.drawLine(0,0,N,0);                         // Mittellinie ziehen
